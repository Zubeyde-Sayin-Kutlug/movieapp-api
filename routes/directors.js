...
router.get("/:directorId", function (req, res, next) {
  directorModel
    .findById(req.params.directorId)
    .then((data) => {
      if (!data) next({ message: "The director was not found.", status: 404 });
      res.json(data);
    })
    .catch((err) => {
      res.json(err);
...
router.put("/:directorId", (req, res, next) => {
  directorModel
    .findByIdAndUpdate(req.params.directorId, req.body, { new: true })
    .then((data) => {
      res.json(data);
    })
    .catch((err) => {
...
router.delete("/:directorId", (req, res, next) => {
  directorModel
    .findByIdAndRemove(req.params.directorId)
    .then((data) => {
      res.json(data);
    })
    .catch((err) => {
